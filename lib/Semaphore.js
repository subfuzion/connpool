 ...
 * acquired is when the requestor is called back without an error.
 */
class Semaphore {
  ...
  acquire(timeout, context, callback) {
    ...
    if (needsTimer && timeout != Infinity) {
      ...
      // save the timer so it can be cleared if the caller is able to successfully acquire
      // before the timeout (if not cleared, outstanding timeouts delay process exit)
